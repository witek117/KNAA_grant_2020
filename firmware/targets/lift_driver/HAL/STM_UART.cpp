...
STM_Uart* uartHandlers[3];

STM_Uart::STM_Uart(USART_TypeDef* uart, uint32_t baudrate) :
...
void STM_Uart::read(uint8_t* buffer, uint16_t length){
    if(buffer != nullptr){
...
bool STM_Uart::isBufferEmpty(){
    return RingBuffer_IsEmpty(&rxRingBuffer);
}

uint16_t STM_Uart::getBufferLevel(){
    return RingBuffer_GetLen(&rxRingBuffer);
}

void STM_Uart::flushRxBuffer(){
...
void UART_IRQ(STM_Uart* uartHandler) {
